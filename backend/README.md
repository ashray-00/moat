...
- Free: Agent on by default (`FREE_AGENT_ENABLED`); Ask+Agent share the monthly
  ask quota. Daily USD caps (`DAILY_COST_USD_*`), Agent round/cost caps,
  disposable-email + signup-rate gates — see root `.env.example` and
