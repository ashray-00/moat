...
async def invite_member(body: InviteBody, user_id: RequiredUser):
    ...
    if n >= limit:
        raise HTTPException(403, f"Seat limit reached ({limit}).")
    return await orgs.create_invite(org["org_id"], body.email, user_id)


@router.post("/accept")
async def accept_invite(body: AcceptBody, user_id: RequiredUser):
    try:
        org = await orgs.accept_invite(body.invite_id, user_id, body.email)
